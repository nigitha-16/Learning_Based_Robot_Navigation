...
class CriticModel_withLaser:
    ...
    def _create_model(self):
        ...
        # Laser processing
        laser_hidden = Dense(64, activation='relu', kernel_initializer=HeNormal())(laser_normalized)
        laser_hidden = Dense(32, activation='relu', kernel_initializer=HeNormal())(laser_hidden)
    
        # Goal processing
        goal_hidden = Dense(16, activation='relu', kernel_initializer=HeNormal())(goal_normalized)
        goal_hidden = Dense(32, activation='relu', kernel_initializer=HeNormal())(goal_hidden)
    
        # Motion command processing
        motion_command_hidden = Dense(8, activation='relu', kernel_initializer=HeNormal())(motion_command_normalized)
        motion_command_hidden = Dense(32, activation='relu', kernel_initializer=HeNormal())(motion_command_hidden)
    
        # Concatenate processed features
        concatenated = concatenate([laser_hidden, goal_hidden, motion_command_hidden])
    
        # Fully connected layers after concatenation
        hidden = Dense(64, activation='relu', kernel_initializer=HeNormal())(concatenated)
        hidden = Dense(32, activation='relu', kernel_initializer=HeNormal())(hidden)
